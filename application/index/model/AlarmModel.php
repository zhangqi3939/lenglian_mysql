...
class AlarmModel extends Model
{
    ...
    public function getCurrentAlarm($params){
        ...
        }elseif($params['USER_LEVEL'] == 1){//agent
            //$this->db->where('c.agentID',$params->agentID);
            $where .= " and C.AGENT_ID = ".$params['AGENT_ID'];
        }elseif($params['USER_LEVEL'] == 0) {//client
           // $this->db->where('c.clientID',$params->clientID);
            $where .= " and C.CLIENT_ID = ".$params['CLIENT_ID'];
        }elseif($params['USER_LEVEL'] == -1){
           // $this->db->where('c.branchID',$params->branchID);
