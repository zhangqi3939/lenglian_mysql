...
//        $mc8k->where('field73729',$box_id)->update('newconfig',$post_data);
    ...
    public function box_list()
    {
        ...
        $User_info['keyword'] = input('post.keyword');
        $User_info['boxGroupID'] = input('post.groupID');
...
//        app_send($result);
    ...
    //添加设备到分组
    function group_box_save(){
        $data = input('post.');
        $User =  new UserModel();
        $Device =  new DeviceModel();
        $channel = 'web';
        $User_info = $User->checkToken($channel);
        $User_info['groupID'] = input('post.groupID');
        ...
        $result = saveBoxToGroup($User_info['groupID'],$User_info['groupBoxIDs'],$User_info['exclusive']);
        if($result){//成功
            //更新分组设备数量
