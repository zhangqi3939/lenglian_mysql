...
function pageInit(){
	...
	window.localStorage.setItem('homePage','warm');
	getUserId();
	monitor_latest();
}
//获取用户id
...
if(!window.localStorage.getItem('indexTableCol1')){
...
function showState(data){
	$('.stateBtn .all span').text(getStateNum1(data).allNum);
	$('.stateBtn .onLine span').text(getStateNum1(data).onLineNum);
	$('.stateBtn .outLine span').text(getStateNum1(data).outLineNum);
//	$('.stateBtn .dormant span').text(getStateNum(data).dormantNum);
	$('.stateBtn .alarm span').text(getStateNum1(data).alarmNum);
}
//点击状态按钮切换表格显示
...
/*function zoneMsg(data){
	$(".msgList .zone").empty();
	var zoneNum = Number(getStateNum1(data).alarmNum);
...
function needMonList(){
	...
	if(num != 0){
		$('.msgList .more').css('color','red');
		if($('#msgList>img').attr('src')=='../img/p.png'){
			$('#msgList>img').removeClass('load');
			$('#msgList>img').trigger('click');
		}
	}
	$('.msgList .more').html('需要监控（' + num + '条）<img src="../img/zhankai.png" width="10px"/>');
}
//消息列表报警消息、行驶消息
// function msgList(){
// 	$.ajax({
