...
function makeInfoWindow(box,address){
	...
	info += '<tr><td class="b">行驶速度：</td><td>'+ Math.round(box.speed * 100)/100 +'Km/h</td>';
	info += '<td class="b">箱内温度：</td><td>'+(isNaN(((Number(box.gps_temp2)+Number(box.gps_temp3))/2).toFixed(1))?'-':((Number(box.gps_temp2)+Number(box.gps_temp3))/2).toFixed(1))+'°C</td></<tr>';
	info += '<tr><td class="b">设定温度：</td><td>'+box.cooler_set_temp+'°C</td>';
	info += '<td class="b">环境温度：</td><td>'+box.ambient_temp+'°C</td></tr>';
	info += '<tr><td class="b">出风温度：</td><td>'+ box.out_air_temp+'°C</td>';
	info += '<td class="b">回风温度：</td><td>'+ box.re_air_temp +'°C</td></tr>';
	info += '<tr><td class="b">机组模式：</td><td>'+runInfo+'</td>';
	info += '<td class="b">电瓶电压：</td><td>'+ box.gps_voltage+'V</td></tr>';
